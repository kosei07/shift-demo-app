...
const AddStaff = (props) => {
    ...
    const inputFirstName = (event) => {//姓の入力時の処理
        setStaffFirstName(event.target.value)
    }

    const inputLastName = (event) => {//名の入力時の処理
        setStaffLastName(event.target.value)
    }
    

    if (props.openAddStaff) {//スタッフ管理画面でスタッフリストからスタッフを選択されたときの処理
        return (
            <div className="overlay modal-center">
                ...
                <div className="multiple_buttons_wrap">
                    <PrimaryButton
                        label={"閉じる"}
                        onClick={() =>{props.setOpenAddStaff(false)}}
                    />
                    <PrimaryButton
                        label={"追加"}
                        color="secondary"
